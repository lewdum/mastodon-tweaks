(() => {
  ...
  async function inject() {
    ...
    debug = settings['debug'] as boolean | undefined
    ...
    const optionDebug = await createBoolOption(
      'debug',
  ...
  async function createBoolOption(
    ...
    const value = data[id] as boolean | undefined ?? defaultValue
  ...
  async function createStringOption(
    ...
    const data = await browser.storage.local.get(id)
    const value = data[id] as string | undefined ?? defaultValue
